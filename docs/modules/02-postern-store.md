...
**迁移（`migrate` 模块）**：以 `PRAGMA user_version` 标识 schema 版本。**怎么做**——开库后先读 `user_version`，与当前实现内置的"最高已知版本"比对，分三态处置：①相等 → 无操作；②**低于**当前（旧库）→ 从库版本号起逐级施加有序的前向迁移步直至追平最高已知版本，全程**包在单一事务内**（任一步失败即整体 ROLLBACK、`user_version` 不前进，杜绝迁移到一半的半截 schema），追平后才前进 `user_version`；③**高于**当前（库由更新实现写过）→ **fail-closed 拒绝**，绝不按旧假设解析未知 schema（防"用旧实现误读新库结构而悄然破坏数据"，对齐 §6.2 boot 的 fail-closed 拒启）。**为什么只前向、不回退**：降级解析未知结构与公理二相悖，故未知高版本只拒不猜；迁移单事务化是"要么整套新 schema、要么原样旧库"的原子性前提，与本域"无半截状态"的一贯纪律同源。
...
> **为什么这样组织**：数据面求值在 `Arc<PolicySnapshot>` 上零库访问查表（§3.4），故 policy.db 的运行期读路径**只剩快照重建**一处——`PolicyRepo` 不承担逐请求查库职责，库的访问被收敛到"写"与"重建快照的全量读"两类，二者又同处一把写锁临界区（见 §3.6）。这使写串行化代价被摊薄（写本就是低频管理命令），且彻底消除了"求值时一边写一边读半截状态"的竞态——求值看到的永远是某次提交后的完整原子投影，而非进行中的库。
...
**授权空间物化的关键展开步**（"物化"具体做什么）：构建时把 `bindings × role_capabilities`（沿 `role_inherits` 展开角色继承的传递闭包）与 `binding_scope` 的辖区合成具体授权格；其中 `binding_scope.kind = selector` 的**标签选择器在此刻按当时的 `resource_labels` 展开为具体资源集**（展开结果落进快照，使求值仍是纯内存查表，不在求值期再算选择器匹配）。**fail-closed**：选择器语法无法解析或展开为空集 ⇒ 该绑定不授予任何资源（空集、不报错也不放行，对齐 5.2bis-②）。角色继承展开须按有限深度处理，遇环不放大授权（继承图无环由模型层校验，构建侧不依赖其成立，仍以"已访问集"去重防环兜底）。
...
本小节把上述功能落地时的并发模型、错误处理、性能边界、测试与可观测性讲清，作为开发的工程基准。与《详细设计文档》第七部分全局规范一致处只引用、不重抄。

**并发与线程模型**——同步 rusqlite + 写串行化，读只在重建：

- **rusqlite 同步、不引 async 库内 IO**：policy.db 经 rusqlite 同步 API 访问，本域不在 crate 内开 tokio 任务、不持 runtime；调用方（daemon::control）若运行于异步上下文，由其在 `spawn_blocking` 边界外承接同步调用，本域只暴露同步签名（库 crate 的纪律：不替调用方决定并发模型）。
- **写串行化 = 单把写互斥锁**：全部 `PolicyRepo` 写（事务）经一把进程内写互斥锁串行执行，杜绝两个写事务交叠。**为什么用应用层互斥锁而非仅靠 SQLite 自身串行**：写锁的临界区不止覆盖"事务 COMMIT"，还要把紧随其后的**快照重建 + Arc 原子替换**包进同一临界区（见 §3.4 / §7-13），这段"提交→重建→替换"必须对外表现为单一不可分原子步，唯有应用层锁能划定这个比单条 SQL 事务更大的临界区。
- **WAL 模式的取舍**：policy.db 开 WAL（§3.2），使"重建快照的全量读"与"下一笔写事务"不互相长时间阻塞读写——但本域并不依赖 WAL 提供的读写并发来支撑多读者，因为运行期唯一读者就是快照重建（仍在写锁内）。WAL 在此主要换来崩溃一致性与 checkpoint 行为可控，而非并发吞吐。
- **审计写与策略写各自独立**：`JsonlAuditSink` 是 append-only 文件追加，不走 policy.db 写锁；数据面内核（多请求并发）对审计的 `record` 是高频并发写入点，其内部串行化（追加顺序、fsync 批次）由 sink 自持，与 `PolicyRepo` 写锁解耦——二者是两条独立载体，绝不互相阻塞（策略写不该被审计洪峰拖慢，反之亦然）。

**错误处理与传播**——每 crate 一个 thiserror 枚举，fail-closed，不吞错（遵《详细设计》7.1 错误模型 / 7.2-5）：

- **rusqlite 原始错误就地归类、不外泄**：底层 `rusqlite::Error` 在 `base`/`PolicyRepo` 内即映射为本域错误枚举的语义变体，**不把原始驱动错误串透出 crate 边界**（与机密红线同理：库路径或 SQL 片段不进上层错误文案）。关键的两类映射须语义分明——①**乐观锁影响行数为 0** 映射为**版本冲突错误**（独立变体，控制面据此返 `409 Conflict`，见 §6.4），绝不与"行不存在/IO 失败"混为一谈；②约束违反（partial unique、CHECK、限制性表 `enable_flag≠1`）映射为对应的**契约/校验拒绝错误**，fail-closed 拒写、库不变。
- **审计错误是独立通道**：`JsonlAuditSink::record` 返回 `AuditError`（core 定义），本域**只如实返回写入成败**，绝不在本域把审计失败自行翻译成放行/拒绝——放行与否的处置在数据面内核（§6.3 / §7-16）。本域对 `AuditError` 不做任何"吞掉当作成功"的降级。
- **错误变体 → 拒绝阶段的穷尽映射在 core**：本域错误进入求值/控制面后的"映射到哪个拒绝阶段"由 core 的穷尽 match 承接（新增变体不写映射即不可编译），本域只负责产出语义正确的变体。
- **panic 政策**：遵 7.1——`unsafe forbid`、clippy 禁 `unwrap/expect/panic/indexing_slicing/arithmetic_side_effects`；本域不靠 panic 表达任何失败，迁移版本不识别、约束违反、IO 失败一律走 `Result` 的 `Err`。数据面侧的 CatchPanic 兜底在内核外壳，不在本域。

**性能与资源边界**——可量化约束：

- **快照重建复杂度**：一次事务内全量加载 + 角色继承展开 + 标签 Scope 选择器展开 + 授权空间物化，复杂度随策略状态总行数线性、随角色继承深度有限放大；这是事件驱动（写/sweeper/import 触发）而非逐请求开销，故可接受全量重建。代价用"写低频 + 求值零库访问"两头摊薄：求值在内存查表为微秒级、零 IO。
- **审计扫描量级**（对齐《详细设计》5.3 的 D2 核算）：单 Agent 高频约 10⁴ 事件/日、每事件 ≤1 KB ≈ 10 MB/日；拒绝分析典型 7 日窗口 ≈70 MB，按日期文件**倒序顺序扫描 + serde_json 逐行解析 + 分页窗口截断**在百毫秒级完成——这正是选 JSONL 而非给审计建库的依据（此量级 SQLite 索引无实质收益，却失去物理 append-only / `tail -f` / 零迁移）。扫描**绝不全量读入内存**：边读边按分页窗口截断、命中页满即停（契约 `DB_PAGINATION_MANDATORY` 对 `scan` 同样生效）。
- **内存边界**：常驻内存主要是当前 `Arc<PolicySnapshot>`（数量级 = 策略状态规模，非请求量），旧快照在最后一个读者 `Arc` 释放后回收；审计写按事件流式追加、不在内存堆积，扫描按页持有。无随请求量增长的无界缓存。
- **超时与配额**：审计落盘有独立配额与水位（§3.5 DoS 防护），逼近上限触发可感知降级而非瘫痪；时间戳生成、`< now` 比较等热点不做堆分配。

**测试策略**——内存/临时真实库分层，对齐 §8 的判定口径：

- **`base`/`PolicyRepo` 逻辑测试用临时真实 SQLite**：审计字段自动填充、乐观锁版本自增、逻辑删除、级联、partial unique、`mode_state` 全局哨兵、`CHECK` 兜底这些落库行为必须在**真实 SQLite**（`tempfile` 临时库或 `:memory:` 内存库）上断言"给定输入→落库行的确切形态"，因为它们的正确性恰恰依赖真实 SQL 语义（约束、索引、事务回滚），用 Fake 无法验证。L-2b 类"绕开写校验直插库再看快照"的反例同样需要真实库。
- **快照消费侧解耦**：策略引擎对快照的求值测试以**纯内存 `PolicySnapshot`** 驱动（core 零 IO 的收益），那是消费域的事，不在本域；本域只测"从库到快照"的构建侧（加载条件分两类、悬挂引用过滤、同辖区多模式取最严格并写告警）。
- **审计 sink 用临时目录**：`JsonlAuditSink` 测按日轮转、append-only、fsync 策略、倒序分页扫描，用临时数据目录验证文件落点（`<data_dir>/audit/YYYY-MM-DD.jsonl`）与"只追加、无行级改写"。
- **契约测试是边界判据的载体**：B 类（`DB_*` / `ARCH_FORBIDDEN_EDGES` / `SEC_ADMIN_NOT_GRANTABLE`）经 Stele 扫描真实代码 + 反例自检判绿红（机制见 7.6），开发不另写等价单测复刻这些静态判定。
- **失败路径必须可注入**：启动 fail-closed（开库失败/版本不识别/首快照失败/审计目录不可写）、事务回滚、审计 `record` 返 `Err` 这些路径要可被测试注入触发（对齐 L-5/L-6/L-12/L-13），验证"半截状态不存在、未审计中间态不存在"。

**可观测性**——本域不产业务运行日志，审计是载体不是本域自身的日志（机密红线遵 7.5）：

- **本域不写 tracing 业务日志**：策略读写、快照重建的逐条细节不进运行日志——逐请求/逐变更的问责痕迹是**审计事件**（observed fact），由控制面/内核在其路径上经本域 sink 落地；本域作为载体只如实追加，**不把"我写了一行 policy.db"再记一条运行日志**（避免双轨与噪声）。tracing 仅用于本域自身的生命周期/异常（开库、迁移、快照重建失败、审计配额降级告警），限于运维诊断级别。
- **审计内容的机密红线在本域之外保证、本域不破坏**：写入审计的字节在内核出口已过 Sanitizer，本域只落**已脱敏**事实（§7-16）；本域自身的错误文案、迁移日志、降级告警同样**不得回显**真实地址、凭据值、SQL 中的业务数据或库路径（与 7.5 内容红线一致）。`policy_change` 审计携带"表名 + 行 id + 写后 version"供与权威库逐行对账，但变更摘要中**凭据值除外**（由观测面记录纪律 8.9 定义，本域只如实承载该已脱敏摘要）。
- **可对账锚点**：`policy_rev`（快照重建时递增的策略修订号）由控制面写入 `policy_change` 与 `request` 事件，支撑"相同请求 + 相同策略 → 相同决策"的复现对账——本域提供"重建即递增"的修订号语义，是这条对账链在存储侧的落点。

> 与全局工程规范的衔接：本小节的并发/错误/lint/测试/日志细则均在《详细设计文档》7.1（代码与错误模型/panic）、7.2（安全红线）、7.3（测试与契约）、7.5（运行日志红线）、7.6（契约接线）框架内，此处只点出**本域特有的落地取舍**（写串行化临界区的范围、rusqlite 错误就地归类、审计与策略写双载体解耦、快照重建复杂度与审计扫描量级），不复述全局条款。
