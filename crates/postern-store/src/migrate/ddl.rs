...
//!
//! 当前最高版本为 2：v0→v1 即"空库建全套表"（[`schema::SCHEMA_SQL`](crate::schema::SCHEMA_SQL)）；
//! v1→v2 建持久 `policy_meta` 键值表（承载单调 `policy_rev`）。新增 schema 演进时在此
//! 追加有序步并抬升最高版本常量。
//!
...
/// 新增演进时在此追加有序步并抬升 [`CURRENT_SCHEMA_VERSION`]。
